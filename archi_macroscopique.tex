...
\paragraph{}
\index{linux}Linux est un noyau monolithique modulaire. Il possède donc une grande richesse de fonctionnalités.\\
Le schéma \ref{fig:linuxmap}, tiré du site {\it makelinux}
(\url{http://www.makelinux.net/kernel\_map}), montre à quel point le noyau Linux est complexe et
...
\label{tab:linux_dirs}
 ...
 \begin{minipage}{0.7\linewidth}
 \vspace{2mm}
 Implémentation de l'ensemble des algorithmes cryptographiques ainsi que de la
 ...
 \begin{minipage}{0.7\linewidth}
 \vspace{2mm}
 Ensemble des drivers de périphérique. Ces derniers sont répartis par famille (\index{ACPI}acpi, net,
 usb, video...)
 \vspace{2mm}
 \end{minipage}
 \\
 ...
 \begin{minipage}{0.7\linewidth}
 \vspace{2mm}
 Ensemble des \index{firmware}firmwares et des éléments bas niveau nécessaires pour s'interconnecter
 ...
 \begin{minipage}{0.7\linewidth}
 \vspace{2mm}
 Implémentation des différents \index{filesystem}filesystems. On y retrouve \index{FS!Ext2}ext2,
 \index{FS!Ext3}ext3, \index{FS!Ext4}ext4, \index{FS!NTFS}ntfs, \index{FS!VFAT}vfat,
 \index{FS!ProcFS}procfs, \index{FS!SysFS}sysfs ou encore la sucrcouche générique à tous : \index{FS!VFS}vfs
 \vspace{2mm}
 \end{minipage}
 \\
 ...
 spécifique à une architecture. On retrouve ainsi la gestion de l'\index{FS!InitramFS}initramfs ou encore le montage
 ...
 \begin{minipage}{0.7\linewidth}
 \vspace{2mm}
 Implémentation des \index{IPC}IPC \index{POSIX}POSIX (Inter-Process Communications) : les
 ...
 générique de gestion de la \index{MMU}MMU.
 \vspace{2mm}
 \end{minipage}
 \\
 ...
 couche 2 à la couche 4 de la pile \index{OSI}OSI. On y retrouve la pile \index{Net!IPv4}IPv4,
 ...
 \begin{minipage}{0.7\linewidth}
 \vspace{2mm}
 Ensemble des éléments de durcissement standard du noyau Linux (\index{SELinux}SELinux,
...
{\bf scripts} &
 ...
 les élements associés. \index{User Mode Linux}UML (User Mode Linux), qui n'est pas de la
 \index{virtualisation}virtualisation à proprement parlé, est géré dans {\it arch}.
 \vspace{2mm}
 \end{minipage}
 \\
\end{supertabular}
...
Pour compenser ce problème, un système de fichier virtuel a été créé, le \index{FS!RamFS}ramfs, dans lequel sont
placés les modules nécessaires au démarrage de la machine. Il est créé sous forme d'un fichier
disjoint du fichier noyau. C'est le \index{bootloader}bootloader qui s'occupe de fournir au noyau un accès à ce
...
Initramfs est le successeur de l'\index{initrd}initrd, dont le but est le même, mais ayant moins de capacité. En
...
\begin{itemize}
\item {\bf \index{init}init}, afin de lancer le premier processus du système, qui à son tour se chargera de
démarrer les autres processus
\item {\bf \index{mount}mount}, afin de monter les possibles autres partitions du système
\item les fichiers de configuration contenus dans {\bf etc}, comme la table des systèmes de fichiers
({\tt \index{fstab}fstab}, la liste des comptes utilisateurs systèmes fichier {\tt passwd} et d'autres éléments
nécessaire à l'initiatisation du système.
\item {\bf les \index{module}modules}, présents dans le répertoire {\tt /lib}, sous forme d'une arboresence de
...
sous forme de module (et sous forme statique également), la liste des périphériques qu'il supporte
...
connaître les opcodes nécessaire à la gestion des éléments de base comme la \index{MMU}MMU, les
\index{DMA}DMA ou encore le frequency scaling. Il est également nécessaire de configurer correctement le timer, si cela n'a
pas été fait par le \index{bootloader}bootloader.
...
mmap}. De plus, la \index{MMU}MMU fournit un cloisonnement mémoire nécessaire au bon fonctionnement de la
...
\paragraph{}
Enfin, les contrôleurs \index{DMA}DMA maître, souvent présents directement dans le processeur, doivent être
configurés. Pour finir, le support du bus \index{PCI}PCI et au travers de lui la découverte des périphériques
...
d'interruption est en général une liste d'adresses. Dans le cas des processeurs de la famille
\index{IA32}IA32,
...
redéfinissable par une tache : \index{IPC!Signaux!SIGKILL}SIGKILL et \index{IPC!Signaux!SIGTERM}SIGTERM et
...
\begin{itemize}
\item un identifiant, nommé {\it \index{Pid}pid} (Process IDentifier)
...
\paragraph{}
L'implémentation de la pile USB commence à se stabiliser ces deux dernières années. Elle reste
cependant très dynamique, du fait de l'arrivée de la norme USB 3 et de la constante intégration de
...
(support des utilisateurs et des groupes, sur une valeur octale allant de 000 à 777),
avec une politique de sécurité de type DAC\footnote{Discretionary Access Control} et le support des
{\it capabilities} POSIX.\\
Les capabilities définissent des classes de privilèges avec le support de l'héritage des privilèges
lors de la création d'un processus fils).\\
\`{A} cela est ajouté le support des droits étendus (flags setuid, setgid, etc).

\paragraph{}
La gestion des droits telle que définie ci-dessus est toujours présente dans un noyau Linux, quel
que soit sa configuration (hors capabilities, qui peut être désactivé). Cependant, il existe
plusieurs modules complémentaires permettant d'intégrer des fonctions de sécurité supplémentaires.
Il s'agit des éléments suivants :\\
\begin{itemize}
\item {\bf SELinux}: introduit par la NSA, ce module fournit un support du mode
RBAC\footnote{Role-Based Access Control} permettant de définir des rôles et des droits pour chacun
deux. Contrairement au DAC, ce n'est plus l'utilisateur qui définit ses droits (et ceux de ses
fichiers) mais la politique de sécurité associé au modèle RBAC.
\item {\bf SMAC}: Il s'agit d'un module intégré récement, dont le but et de supporter un modèle de
sécurité de type MAC\footnote{Mandatory Access Control} simplifié. Il s'agit de définir de manière
statique des droit pour les différents utilisateurs en terme de visibilité sur le système et sur
les accès qu'il a le droit de faire. Il s'agit d'un modèle plus simple à gérer que le RBAC, mais
moins précis.
\item {\bf AppArmor}: Module intégré dans le noyau Linux par Canonical (distribution Ubuntu) qui
s'en sert dans sa distibution. Il fournit le support d'un modèle RBAC avec support de
...
éléments matériel de sécurisation.
\end{itemize}

\paragraph{}
Linux implémente un certain nombre d'algorithme de chiffrement et de hachage, comme le SHA1, l'AES
ou encore le DES. Ces différents algorithmes sont employés au niveau du noyau pour la
gestion de l'IPSEC ou des systèmes de fichiers chiffrés par exemple. Ils sont également accessible
aux applicatifs au travers de l'API du noyau.

\subsection{Les pilotes de périphériques}
\paragraph{}
Comme vu dans le cas de l'USB, Linux implémente un grand nombre de pilote de périphérique. Ces
derniers sont classifiés en fonction de leur famille (net devices, usb, char devices, block
devices...). \`{A} chaque famille est associée une liste de pilotes dans laquelle Linux va piocher
lors de la découverte des périphériques.\\
Un pilote de périphérique fournit l'interface entre le noyau et le matériel qu'il gère. Il doit
donc s'enregistrer auprès du noyau pour lui preciser quel matériel il supporte, et comment le noyau
doit se servir de lui. Chaque famille possède une interface standardisée qui permet au pilote
d'enregistrer les différentes fonctions de son API auprès du noyau.\\
Pour définir les périphériques qu'un pilote supporte, le noyau s'appuie sur un système normalisé de
couple constructeur/modèle. Il s'agit de deux valeur numérique que chaque périphérique est capable
de donner de manière générique. Lorsqu'un pilote donne au noyau la liste des périphérique qu'il
supporte, il fournit la liste des couples constructeur/modèle qu'il sait gérer. Ce couple est bien
sur unique pour un modèle donné. Pour pouvoir posséder un numéro de constructeur, la société doit
s'enregistrer auprès d'instances internationales.
